                        ...
                        <form action="" method="POST" onsubmit="document.getElementById('pay').innerHTML = 'Processing...';">
                            ...
                            if(isset($_POST['submit'])){
                              if($_POST['destination'] == ""){
                                     echo'<div class="alert alert-danger">Please select destination</div>';
                              }elseif($_POST['amount'] == ""){
                                ...
                                if($amount > $earnings){
                                  ...
                                  echo'<div class="alert alert-danger">insufficient referral bonus. Bonus: '.$earnings.'</div>';
                                    ...
                                    if($destination == "vtu"){
                                    ...
                                    }elseif($destination == "earning"){
                                         // transer to user account in database
                                        $fetchet = mysqli_fetch_assoc(mysqli_query($database,"SELECT * FROM accounts_mlm WHERE username = '$paidname'"));
                                        $bonus_add = $fetchet['earnings'] + $amount;
                                        
                                        $transfer =  mysqli_query($database, "UPDATE accounts_mlm SET earnings = '$bonus_add' WHERE username='$paidname'");
                                    }
                                        ...
                                        $date = date("Y/m/d - h:i:sa");
                                          ...
                                          mysqli_query($database, " INSERT INTO `mlm_notification` (`id`, `username`, `email`, `transactionID`, `transaction_ref`, `status`, `type`,  `amount`, `date`) 
                                            VALUES ('', '$paidname', '$paidemail', '$id', '$ref', 'succesful',  'bonus_transfer', '$amount', '$date');");
                                           
                                        
                                        // subtract fund from user
                                        $bal = $earnings - $amount;
                                        
                                         $upd1 =  mysqli_query($database, "UPDATE accounts_mlm SET referral_bonus = '$bal' WHERE user_id='$user_id'"); 
                                          
                                          echo'<div class="alert alert-success">Bonus successfully transfered too the destination you selected</div>';
                              ...
                              <div class="row">
                                   ...
                                   <div class="col-md-12">
                                      <div class="form-group">
                                        <label class="font-weight-700 font-size-16">Selet Destination</label>
                                        <select name="destination" class="form-control">
                                              <option value="">--------------------</option>
                                              <option value="earning">Add to your MLM balance</option>
                                              <option value="vtu">Add to your vtu wallet (to buy our services)</option>
                                        </select>
                                      </div>
                                  </div>
                                   <div class="col-md-12">
